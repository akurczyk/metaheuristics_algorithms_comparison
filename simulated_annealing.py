...
class SimulatedAnnealing:
    def __init__(self, states, no_of_iterations, initial_temperature, cooling_coefficient, inc_support, dec_support):
        self.states = states
    ...
    def probability(self, new_val, old_val):
        return math.e * ((1 - ((old_val - new_val) / old_val)) / self.temperature)
    ...
    def run(self):
        ...
        for i in range(self.no_of_iterations):
            ...
            if solution.value > self.best_solution.value \
                    or (random.randint(1, 1000) / 1000) < self.probability(new_solution.value, solution.value):
